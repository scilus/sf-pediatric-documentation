...
![sf-pediatric pipeline schema](../../../../assets/0.2.1/sf-pediatric-schema.png)

:::note[Don't hesitate to provide feedback!]
This page describe the basic usage for sf-pediatric. If something is not clear for you, do not hesitate to open an issue on [sf-pediatric GitHub page](https://github.com/scilus/sf-pediatric.git).
:::
...
<Steps>
  1. **`-profile segmentation`:**
     ...
     :::caution
     **A valid FreeSurfer license file is required for this profile. Specify the path to your license using [`--fs_license`](/sf-pediatric/0.2.1/guides/parameters#segmentation-options).** If you don't have a valid license, you can register for one [here](https://surfer.nmr.mgh.harvard.edu/registration.html).
     :::
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run scilus/sf-pediatric -r 0.2.1 --input <BIDS_directory> --outdir ./results -profile docker,tracking -resume
    ```
  </TabItem>
</Tabs>
...
<Steps>
  1. **`--input`**: for the path to your BIDS directory
     ...
     For more details on how to organize your input folder, please refer to the [inputs section](/sf-pediatric/0.2.1/guides/inputs).
  ...
  2. **`--outdir`**: path to the output directory
     ...
     We do not specify a default for the output directory location to ensure that users have total control on where the output files will be stored, as it can quickly grow into a large number of files. The recommended naming would be something along the line of `sf-pediatric-v{version}` where `{version}` could be `0.2.1` for example.
...
:::note
More parameters can be tuned to your own usage, for a concise list of the most common parameters, you can run `nextflow run scilus/sf-pediatric -r 0.2.1 --help`. Otherwise, for the complete list of parameters, please refer to the [parameters section](/sf-pediatric/0.2.1/guides/parameters) of the documentation.
:::

:::caution[Running the pipeline in offline mode]
If the computer nodes you are planning to use do not have access to the internet, there is a few steps to go through to make sure the containers and templates are available during pipeline execution. Please head over to [running with no internet access section](/sf-pediatric/0.2.1/guides/nointernet).
:::
...
<FileTree>
  ...
  * sf-pediatric-v0.2.1/            # Finished results in specified location (defined with --outdir)
    * multiqc/
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run scilus/sf-pediatric -r 0.2.1 -profile docker -params-file params.yaml
    ```
  </TabItem>
</Tabs>
...
First, go to the [scilus/sf-pediatric releases page](https://github.com/scilus/sf-pediatric/releases) and find the latest pipeline version - numeric only (eg. `0.2.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 0.2.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
