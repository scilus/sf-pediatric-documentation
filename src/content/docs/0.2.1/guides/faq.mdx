...
pipeline. To avoid reaching the disk quota, we suggest you run the pipeline in chunks, starting with the [`tracking` profile](/sf-pediatric/0.2.1/guides/usage/#choosing-a-profile).
Then, add the [`--input_deriv`](/sf-pediatric/0.2.1/guides/parameters/#inputoutput-options) parameter to tell the pipeline where to fetch its
