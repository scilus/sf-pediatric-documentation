...
Some computing nodes do not have access to the internet at runtime. Since the pipeline interacts with the container repository and containers are pulled during execution, you will get errors if the computing nodes do not have access to the internet. Fortunately, containers can be downloaded prior to the pipeline execution, and fetched locally during runtime. To facilitate usage in this specific use case, we provide a download script that handles all container downloads that will be required at runtime.
...
<Steps>
  ...
  2. **Run the download script**
     ...
     <Tabs>
       <TabItem label="Command">
         ```bash
         curl -fsSL https://raw.githubusercontent.com/nf-neuro/modules/main/assets/download_pipeline.sh | bash -s -- -p scilus/sf-pediatric -r 0.3.0 -d 1 -o ./containers -c ./singularity_cache
         ```
       </TabItem>
       ...
       <TabItem label="Expected Output">
         ...
         done - revision: 17bb3e73e0 [0.3.0]
  ...
  3. **Download the templates**
     ...
     **If you are planning to submit your main orchestrator job (the session that will dispatch the different processes)**,
     you also need to predownload the templates used by `sf-pediatric`. If you plan on running the pipeline within a `tmux` session
     on a login node, this is not required, the pipeline will be able to download the templates automatically. To do so manually,
     please run the following commands:

     <Tabs>
       <TabItem label="Command">
         ```
         mkdir -p $HOME/.cache/sf-pediatric
         curl -fSL https://osf.io/Kd8h7/download -o templates.tar.gz
         tar -xzf templates.tar.gz -C $HOME/.cache/sf-pediatric
         rm templates.tar.gz
         ```
       </TabItem>
     </Tabs>

     When you will launch the pipeline, it will automatically look in `$HOME/.cache/sf-pediatric` for the required templates.
</Steps>
...
<Steps>
  ...
  2. **Launching `sf-pediatric` using the SLURM scheduler**
     ...
     Create your `params.yml` file containing your parameters for this specific pipeline run (if not already done).
     ...
     Finally, you can launch the pipeline (see the [Running the pipeline](/sf-pediatric/0.3.0/guides/usage) section for more details) using a simple call to nextflow:
     ...
     :::tip[Have access to a GPU?]
     If you have access to GPUs, simply add `gpu` to your list of profiles (e.g., `-profile ...,slurm,apptainer,gpu`).
     This will speed the pipeline execution by leveraging GPUs when possible. For more details, please see [this section](/sf-pediatric/0.3.0/guides/profiles/#gpu).
     :::
...
<Tabs>
  <TabItem label="cmd_sfpediatric_sbatch.sh">
    ...
    # Call for the pipeline execution.
    nextflow run scilus/sf-pediatric -r 0.3.0 \
        -params-file params.yml \           # Don't forget to create your params.yml file!
        -profile ...,apptainer,slurm \      # Add any processing profiles you want to run.
        -resume
    ```
  </TabItem>
...
:::caution[Don't forget to create your `params.yml` file!]
The previous examples assume you created a sidecar `params.yml` file containing all of your pipeline parameters. If
not already done, please create one by following the instructions [in this section](/sf-pediatric/0.3.0/guides/usage/#how-to-use-a-paramsyml-file).
:::

### **Downloading TemplateFlow templates**

`sf-pediatric` allows users to select a specific output space in which all subjects will be registered. These templates are fetched using the [TemplateFlow Archive](https://www.templateflow.org/) [(Ciric *et al*, 2022)](https://doi.org/10.1038/s41592-022-01681-2). While downloading is directly handled in `sf-pediatric` when internet is available, for offline use, the selected template (specified using the [`--template`](/sf-pediatric/0.3.0/guides/parameters#output-to-template-options) parameter) needs to be available. You can refer to the [official TemplateFlow documentation](https://www.templateflow.org/usage/archive/) for information on how to download the archive using either `datalad` or the Python client.

:::note
Similar to the containers, `sf-pediatric` needs to be told where to look for those downloaded templates. This can be done using the [`--templateflow_home`](/sf-pediatric/0.3.0/guides/parameters#output-to-template-options) parameters.
:::
