...
| `frf_min_fa` | If frf\_fa does not extract enough voxels, this is the minimum FA threshold that will be tried. | `number` | 0.5 |  | True |
...
| `show_hidden` | Display hidden parameters in the help message (only works when --help or --help\_full are provided). | `boolean` |  |  |  |
