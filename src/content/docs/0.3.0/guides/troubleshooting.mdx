...
slug: 0.3.0/guides/troubleshooting
---
...
<Steps>
  ...
  4. **Look for specific error messages**
     ...
     Sometimes the error message will be self-explanatory, and you will be able to understand the root of the issue. If not, you can look in the [common errors and solutions](/sf-pediatric/0.3.0/guides/troubleshooting/#common-errors-and-solutions) section for some help.
</Steps>
...
* Verify your BIDS directory structure matches the [inputs guide](/sf-pediatric/0.3.0/guides/inputs)
...
**Solution:**
Add an `age` column to your `participants.tsv` file. See the [inputs guide](/sf-pediatric/0.3.0/guides/inputs#participantstsv-example) for the correct format. Remember that columns must be **tab-separated**, not space-separated.
...
**Cause:** You specified your optional parameter through the command-line rather than via a `params.yml` file.

**Solution:**
Convert your CLI parameters into a `params.yml` file. To do so, follow the instructions on [how to use a `params.yml` file](/sf-pediatric/0.3.0/guides/usage/#how-to-use-a-paramsyml-file).
...
**Solution:**
Specify your shells manually using `--dti_shells` and `--fodf_shells`. See the [shell selection section](/sf-pediatric/0.3.0/guides/usage#shell-selection-for-dti-and-fodf-models) for details. Example:
...
If you have heterogeneous datasets with varying acquisition protocols, you can specify a different threshold for both DTI and fODF fitting. To do so, use the [`--dti_max_shell_value`](/sf-pediatric/0.3.0/guides/parameters/#dti-options) and/or the [`--fodf_min_shell_value`](/sf-pediatric/0.3.0/guides/parameters/#fodf-options).
:::
...
* If you do not have access to the internet, refer to step #3 in [this section](/sf-pediatric/0.3.0/guides/nointernet/#downloading-the-containers)
...
![wrong\_subcortical\_visualization](../../../../assets/QC/0.3.0/failed_subcortical_qc.png)

**Cause:** It is likely that the specified ROIs for the subcortical regions are not correct.

**Solutions:**

* Validate that the specified ROIs for the subcortical regions using [`--subcortical_rois`](/sf-pediatric/0.3.0/guides/parameters/#atlases-options)
