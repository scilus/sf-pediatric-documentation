...
Some computing nodes does not have access to internet at runtime. Since the pipeline interacts with the container repository and pull during execution, you will get errors if the compute nodes do not have access to the internet. Fortunately, containers can be downloaded prior to the pipeline execution, and fetch locally during runtime. To facilitate usage in this specific use case, we provide a download script, that handles all container downloads that will be required at runtime.
...
1. **Make sure you have all prerequisites**
    ...
    The script relies on three dependencies that should already be available on your computer clusters:

    :::note[Loading a module on a compute cluster]
    For example, on Alliance Canada's clusters, some softwares come prebundled into modules that you can easily load without having to install them yourself. 
...
1. **Set environment variables for pipeline execution**
    ...
    Now that all containers are downloaded, we need to tell `nextflow` where to look for those containers. This can be done using environment variables. For example, if you run the pipeline in the same directory as you downloaded the containers, you should set those environment variables:
...
2. **Lauching `sf-pediatric` using the SLURM scheduler**
    ...
    To open a `tmux` session, simply run `tmux` in the desired location. This will open a new session, in which you will run the pipeline as described below. To exit the session without killing it, use the `CTRL-B + D` shortcut. This will close the `tmux` session, but it keep running the pipeline for you. To reattach to this session, use `tmux a`. For more informations on how to use `tmux`, you can refer to this [cheat sheet](https://tmuxcheatsheet.com) or this [introduction](https://www.redhat.com/en/blog/introduction-tmux-linux).
    :::
...
Some clusters do not allow nextflow pipelines to run on login nodes (even though no processing is done on them, only job scheduling and orchestration). In this case, you should run the pipeline within a single SBATCH job. Here is an example, in which we ask for a single node with 40 cpus. 

:::note[No `tmux` is required for single SBATCH jobs]
Since all job management will be done directly on a compute node, we do not need to rely on a multiplexer to keep our job alive. This can also be a advantage of submitting a single SBATCH job if your compute cluster do not allow multiplexers.
:::
