...
behind using custom atlases. Additionally, we included [test results](/sf-pediatric/guides/atlases/#how-good-the-mapping-process-is)
...
For example, the right/left SFG_(L/R)_6_2 in the Brainnetome Child atlas is labeled 3 and 4 respectively.
:::
...
[`--atlas_folder`](/sf-pediatric/guides/parameters#atlases-options) and the
[`--atlas_name`](/sf-pediatric/guides/parameters#atlases-options) parameters. By default, `sf-pediatric` will
...
Starting from the basic example described in the [a simple example section](/sf-pediatric/guides/usage/#a-simple-example), specifying
...
specified using [`--atlas_name`](/sf-pediatric/guides/parameters#atlases-options) as the pipeline does not currently iterate over multiple atlas names.
...
**Don't forget to update [`--subcortical_rois`](/sf-pediatric/guides/parameters#atlases-options) when changing atlases!**
:::
...
Desikan atlas (previously transformed into fsLR-32k space and formatted [as described in the requirements](/sf-pediatric/guides/atlases/#requirements))
...
![atlas mapping comparison](../../../assets/atlas_comparison.png)
